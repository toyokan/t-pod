...
- **起動経路（`index.html` の `init()`）**: **キャッシュから即描画 → 裏で最新を取得 → 生テキストの `===` で
  ...
  - **`#errorRetry` は `location.reload()` しない**（シェルを取り直すと Tailwind CDN とフォントの代金を再度払うので、
    ...
    2 本ずつ張られる（ポップアップが 2 回開く）。**世代番号は非同期のコールバックしか黙らせない**ので、
    `init()` の頭で同期的に走る結線（`registerSW()` / `setupInstallPrompt()`）は `bootstrapped` フラグで
    一度きりにする——素で並べると再試行のたびに `installBtn` の click リスナが増え、「追加」を 1 回
    押しただけで導線が 2 回・3 回と走る（実測: 再試行 2 回でモーダルが 3 回開いた）。`showLinePromo()` も
    同じ理由で `linePromoArmed` を持つ。
  - **見た目の状態は必ず両方向を書く**——「条件が真なら出す」だけを書くと、静かな差し替えで条件が
    偽になった回に前の表示が残る（テーマバナー `#themeBanner`、マスコットの `hidden`／`flex` が該当）。
...
- **和文の字面（本文フォント）**: 本文は **Zen Kaku Gothic New**（Google Fonts。吹き出しだけ DotGothic16）。**読み込むウェイトは 400/500/700/900 の 4 種から増やさない**——和文は unicode-range で 100 以上のサブセットに割れるので 1 ウェイト足すだけで転送量が 25% 増え、`trimRuntime()` の上限にも早く当たる。そのぶん **Tailwind の `fontWeight` は `semibold: 500` / `extrabold: 700` に付け替えてある**（`tailwind.config`）——素のままだと 600/800 は CSS のフォント照合で 700/900 へ吸い寄せられ、`font-semibold`＝`font-bold`・`font-extrabold`＝`font-black` と**見分けが付かなくなる**（4 段のつもりが 3 段になり、10〜13px の和文が 900 で潰れる）。**今の太さを保ちたい箇所は 600/800 を使わず `font-bold` を直接指定する**（ガラスの上の 11px ラベル＝ボトムナビ、ブランド地の上の `#eventSubtitle` がこれ。ここを 500 に落とすと細くて読めない）。**`palt` は見出し（`h1,h2,h3`）だけに掛ける**——本文に掛けると長文で字送りが不均一になって逆に読みにくい。字間は `body` の `letter-spacing: .01em` まで（これ以上広げると「全10プログラム」のような `whitespace-nowrap` の要素がはみ出す）。**`text-rendering: optimizeLegibility` は使わない**——和文は字数が多く、カーニング／合字の照合が会場の非力な端末で高くつくわりに見た目が変わらない。
- **コントラストの制約**: `brandColor` は明るい色（amber・黄）も指定されるため、**ブランド色ベタ塗り＋白文字（`--brand-fg`）を新規に増やさない**（実測 1.5〜4.4:1 で WCAG AA 未達。`updateBrandFg()` の輝度閾値 0.55 は緩く、amber でも「白」を選ぶ）。**ブランド面（`#appHeader` / `.brand-bg` / `.brand-card` / `.grad-pill`）の文字色は `var(--brand-fg)` に従わせる**——白を直書きすると ID セレクタが `text-brand-fg` クラスに勝ち、明るい brandColor でも白のままになる（黄 `#FACD4B` で実測 1.51:1。`tests/test_shell_source.py` が検出する）。強調は**白ピル（淡地）＋ブランド濃色文字**で行い、濃色は **`color-mix(in srgb, var(--brand) 48%, black)`（＝`brand-800` 相当）**を使う（ボトムナビ・日付タブの実装を参照）。**`brand-700`（62%）は文字に使わない**——既定の黄 `#ffd900` では白地に対し 3.55:1 で AA 未達（`brand-800` なら 5.52:1）。**例外はアイコン単体**——非テキストの UI 部品は WCAG 1.4.11 の 3:1 でよいので、色味を立てたいときだけ 62% まで上げてよい（黄 3.55:1 / amber 5.14:1。ボトムナビの選択中アイコンがこれ）。**ラベル文字は必ず 48% のまま**にし、62% をこれ以上明るくしない（黄が 3:1 を割る）。また、**明るいブランド色は淡地（`brand-50`〜`200`）や枠線が白い地から見分けられない**（1.0〜1.7:1）ため、ピルを浮かせるには**グレーのトラック＋白ピル**が要る（日付タブは `bg-slate-200/80`、ボトムナビはカプセル自身の地がトラックを兼ねる）。新しい配色を足すときは、既定の黄 `#ffd900` と amber `#f59e0b` の両方で 4.5:1 以上を実測して確認する（`color-mix` の計算結果は Chrome が `oklab()` 表記で返すことがあるため、canvas に塗って RGB を取るのが確実）。
- **ボトムナビ（iOS 26 Liquid Glass 風の浮いたカプセル）**: `#bottomNav` は**当たり判定を持たない位置決めの器**（`pointer-events-none`）で、実体は中の `#navBar.nav-glass`（`pointer-events-auto`）。画面下端に貼らず左右 `px-3`・下 `--nav-bottom` の余白を取って**浮かせる**ので、本文はカプセルの下と左右を流れる。**下の余白はセーフエリアと `--nav-gap` を足さず、`max()` で大きいほうを採る**——足すと iPhone で 2 つぶん空いて浮きすぎる。さらに `--nav-safe-trim`（0.75rem）でホームインジケータ領域の一部を食い、実測で iPhone 40px → 22px まで下げてある。**`--nav-safe-trim` をこれ以上増やさない**——画面下端からの上スワイプ（ホームへ戻る）と競合する。**ナビの上下位置は `--nav-gap` / `--nav-safe-trim` の 2 つだけで動かす**——`#bottomNav` の `padding-bottom` と `--nav-h` の両方が `--nav-bottom` 経由でここから決まるので、直書きすると 2 箇所がずれる（マスコットと更新トーストが取り残される）。**`--nav-h` はセーフエリアぶんを含む**ので、`#mascot` や `.above-nav` で `env(safe-area-inset-bottom)` を足してはいけない（二重に空く）。**屈折を SVG フィルタで作らない**——`backdrop-filter: url(#…)` は **iOS Safari が非対応**で、主要な閲覧環境である iPhone にだけ効かない表現になる（以前入れていた `#lensGlass` の `feDisplacementMap` はこの理由で撤去）。代わりに**縁だけを残すグラデーションのリング**（`::before` に `padding` ＋ `mask` の xor 合成）と多層グラデーションで光の回り込みを作る。**この `::before` は `@supports ((-webkit-mask-composite: xor) or (mask-composite: exclude))` で必ず囲う**——非対応だと mask が効かずベタ塗りの板になってナビを覆い隠す。**`backdrop-filter` を入れ子にしない**——選択ピル（`.nav-brand-card`）は親の `.nav-glass` の中にあるので、ピルにも置くと背景の合成が 2 回走る。しかもピルは毎フレーム動く（スワイプ追従・スライド）ため、2 回目が指を動かしている間ずっと再実行される。ピルは白 .98〜.86 でほぼ不透明なので見た目の差は**最大 6/255**（濃い背景を真裏に敷いて実測）しかなく、会場の非力な端末には代金だけが残る。**彩度はレンズの「枠」ではなく「レンズ越しの中身」に乗せる**——iOS 26 のタブバーで色がついて見えるのは選択中のアイコンとラベルであって、ピルの縁ではない。縁を虹色にすると枠だけが目立って中身が負けるので、リング（`::before`）は白に色を差す程度に留める。**彩度を CSS で伸ばそうとしない**——`oklch(from … l calc(c * 2) h)` は sRGB の外へ出た分をブラウザが素朴にクリップするため、彩度ではなく**色相がずれる**（実測で既定の黄 18.5°・amber 28.4°。amber は赤へ寄る）。`color-mix(… black)` も sRGB で黒を混ぜるので明度と一緒に彩度まで落ち、濃くはなっても鮮やかにならない。そこで **`applyNavInk()`（`index.html`）が実行時に計算する**——ブランドの色相のまま、白ピル上で下限コントラストを満たす範囲で **sRGB に収まる最大の彩度**を二分探索し、`--nav-ink`（ラベル）/`--nav-ink-icon`（アイコン）に入れる。CSS 側は `var(--nav-ink, color-mix(…))` で受けるので、JS が走る前とブランド色が無いイベントは従来の濃色に落ちる。**彩度の頭打ちは係数ではなく明度で決まる**（暗いほど sRGB に入る C が小さい）ので、コントラスト下限ぎりぎりまで明るくしたところが最も鮮やかになる。アイコンが 3:1、ラベルが 4.5:1 なのでアイコンのほうが濃く出せる。**既定の黄はほぼ頭打ち**（白地の上で鮮やかな黄は成立しない）だが、実際のイベントはいずれも `brandColor` を設定しているので効果は出る（実測: teal `#005860`→`#007c87`、navy `#001f43`→`#006ce1`）。**色収差（シアン／マゼンタの `drop-shadow`）はアイコンにだけ掛ける**——11px のラベルに掛けると「レンズ越し」ではなく印刷の見当ずれに見え、文字自体が読みにくくなる（24px のアイコンは線が太いので分散として読める）。**カプセル自身が「グレーのトラック」を兼ねる**（最下層に `rgba(226,232,240,.76)`）ので、選択ピル（`.nav-brand-card`）は**白ガラス＋ブランド濃色文字**のまま淡いブランド色でも見分けられる。角丸は外 `rounded-full`（≈30px）／内 `rounded-full`（≈24px）で自動的に同心になる。**高さは `:root` の `--nav-h`（カプセル 4.05rem ＋ `--nav-gap`）に集約**し、`#mascot` と `.above-nav`（更新トースト）はここから位置を取るので、**ナビの高さを変えたら `--nav-h` だけ直す**（以前は 5.5rem / `bottom-24` の直書きで、ナビを変えるたびに 2 箇所がずれた）。スワイプの進捗は `#bottomNav` ではなく **`#navBar` の幅**で測る（器は左右の余白ぶん広い）。**`backdrop-filter` のコストは「ぼかしの半径」ではなく「有るか無いか」**——GPU を切って trace を取ると、スクロール 90 フレームのラスタ処理が無し 283ms に対し有り 741ms（2.6 倍）で、半径を 8px まで落としても 9% しか変わらない（非単調でノイズの範囲）。つまり**半径を下げても軽くならない**ので、軽くしたいなら外すこと。非力な端末では head の inline script が `.no-glass` を付けて`prefers-reduced-transparency` と同じ不透明版へ落とす。**判定はメモリとコア数の両方が低いときだけ**にする——コア数はコンテナや省電力で実力より小さく申告されることがあり、片方だけだと十分動く端末から取り上げる。`deviceMemory` は Safari に無いので iPhone は常に対象外（実機でも重くない）。地の不透明度をフォールバック側で戻すときは **`body.is-scrolled .nav-glass`（詳細度 0,2,0）も一緒に打ち消す**——`.nav-glass` だけ不透明にしても後勝ちで透過値が戻り、「ぼかし無しで半透明」といういちばん読めない状態が残る。**地の不透明度は手計算で決めず、実際に描画して測る**——`backdrop-filter` のぼかしが周囲を巻き込むため単純なアルファ合成の計算と合わず、手計算で 4.7:1 と見積もった `.64` は実測 3.94:1（純黒を真裏）で AA を割っていた。現在は静止 `.76`（実測 4.77:1）／スクロール中 `.82`（同 5.18:1）。
- **アニメーション**: 追加してよいのは `transform` / `opacity` のみ（GPU 合成・ビルド工程なしを維持）。タッチ／スクロール系のリスナは **`{ passive: true }` を崩さない**（スワイプは指追従せず「離した時点でコミット」する設計。`index.html` の `TAB_ORDER` とスワイプ IIFE を参照）。新規の演出クラスは `prefers-reduced-motion` ブロックへ必ず追記する（Tailwind の `transition` / `active:scale-*` はこのブロックの対象外なので取りこぼしに注意）。**進行中カードの「回る縁」（`.now-ring`）は角度ではなく `transform` で回す**——`conic-gradient(from var(--angle))` を `@property` で回すと縁を毎フレーム塗り直すことになるので、**リング状に切り抜いた器＋その中で回る大きな四角**（`.now-ring > i`）に分け、回すのは `rotate()` だけにする。中の四角は**幅 140% の正方形**（カードの対角線より大きいので角が覗かない。横長にすると角速度が場所で変わって光の進みがムラになる）。器は `padding` ＋ `mask` の xor 合成でカード枠と同じ 2px の帯だけを残すので、下の `border-brand-500` の上に光だけが重なる（枠が消える瞬間が無い＝進行中の目印は常に読める）。**`inset` は `0` ではなく `-2px`（＝枠幅）にする**——絶対配置の基準はカードの border box ではなく **padding box（枠の内側）**なので、`0` だと光の帯が枠の「上」ではなく「内側」に出て**罫線が二重に見える**。さらに `border-radius: inherit` はカードの外側の半径（16px）なのに padding box の角は 16−2＝14px なので、**角のアールが食い違ってズレる**（実測: リングの箱がカードの border box より左右上下 2px ずつ内側だった）。**一度だけ光る枠（`.is-now::after`）を `prefers-reduced-motion` で `animation: none` にしてはいけない**——止めると `opacity: 1` のまま残り、カードの 3px 外側にもう 1 本の輪が出たままになる（これも二重の罫線）。`display: none` で消すこと。**ナビのリングと同じく `@supports ((-webkit-mask-composite: xor) or (mask-composite: exclude))` で必ず囲う**——非対応だと mask が効かずベタ塗りの板がカードを覆う。**光はブランド色を混ぜた不透明な色で作らない**——`color-mix` の色は枠を「置き換える」ので一周ごとに縁が途切れる（navy `#001f43` で実測、枠そのままの 15.7:1 が光沢のピークで 1.8:1 まで落ちた）。**白と黒の半透明を重ねる**と下の枠の色が必ず残る。**濃さは手計算で決めず、合成後の色を地（`#f8fafc`）と突き合わせて測る**（現在 白 .40 / 黒 .34。光沢のピークで 黄 1.18 / amber 1.52 / coral 1.85 / violet 2.95 / teal 2.97 / navy 4.28）。**白をこれ以上濃くしない**（.70 で navy 1.8・黄 1.07 まで落ちて縁が切れて見える）。陰は逆に枠を濃くするので明るい色ほど効き（黄 1.32→3.04）、**明るい brandColor では陰が、暗い brandColor では光沢が動きを伝える**。見た目の確認は `dev/now-card.html`（ブランド色・`no-glass` を切り替えて並べる）。非力な端末（`html.no-glass`）では回さない——静止した枠だけで「進行中」は伝わる。器は DOM の実体が要るので `setNowRing()` が付け外しするが、**既にあるときは作り直さない**（1 分ごとの `updateNowMarker()` で回転が先頭へ戻る）。
- **アイコン**: 絵文字は使わず **SVG（Lucide系）**。`ICONS` レジストリ + `icon(name, cls)` ヘルパ（`index.html`）。静的箇所（ボトムナビ・FAB・モーダル閉じる）はインラインSVG。
- **マスコット（右下のふせんネコ）**: 造形の定義は **`assets/fuseneko/fuseneko-grid.js` の 1 箇所だけ**（24×24 の文字列配列 `FUSENEKO_BASE` ＋ 表情パッチ ＋ しっぽの振り）。`index.html` は `<script src>` でこれを読み、`buildCatSvg()` が待機アニメのコマを組む。配布用の SVG 資産（`assets/fuseneko/fuseneko-{base,standard,smile,serious}.svg`）は `scripts/build_fuseneko.py` が**同じ定義から生成する**ので手で編集しない（ずれたら `tests/test_fuseneko.py` が CI で落とす）。パッチは**セル単位で重ねられる**形式で、`_` は「下の絵を残す」・`.` は「透過にする」を意味する（`applyPatches()` が合成。しっぽの振りとまばたきを同じコマに同時適用できる）。塗り分けは **`--cat-base` / `--cat-fold` / `--cat-outline` / `--cat-face` / `--cat-cheek` の 5 変数**に集約し、既定値を `--brand` から `color-mix()` で作るのでイベント毎の `brandColor` に自動追従する（`fuseneko.js` の `fusenekoPalette()` が同じ比率を JS 側にも持つ。単体の SVG はフォールバック値を内蔵）。**黒に近い固定色は使わない**（重く見えるため）。ほっぺだけ固定のピンク。**24×24 のまま作り込む**（32×32 にすると 48px 表示で 1 ドット＝1.5px になり、DPR1 でドット幅が 1px/2px に割れる。整数比を保つには 64px 表示が必要で、ボトムナビ上の本文を圧迫する）。造形の決めごと（設定資料に合わせる。**色と表情以外は変えない**）: ①**胴体は col 1–17 / row 5–19 の 17×15**——ほぼ正方形でわずかに横長。幅を奇数にしてあるので中心は col 9 ちょうどで、顔の左右対称が整数で取れる。下辺は水平・左右は垂直・下の角は四角。②**右上は耳を立てず、ふせんの角を下へ折り返す**——top edge の先から `(14,6)(15,7)(16,8)(17,9)` と 1 ドットずつ階段状に切り落とし、内側を `S` で塗って row 9 の横線と col 12 の縦線で閉じる。大きな三角 1 枚にしない。ここがトレードマークなので省略しない。③左耳は col 1–4 / row 2–4 の小さな階段で、右と対称にしない。耳ぴく（`FN_EARS_DOWN`）も**左耳だけ**動かす（紙のフラップがぴくつくとおかしい）。④顔は**目＝1 ドット幅 × 3 ドット高の細い縦棒**（col 5 / col 13）、**口＝上段の 3 点（col 7 / 9 / 11）＋下段の 3 ドットの帯（col 8–10）**で ω を作る。**鼻はほっぺの色ではなく目・口と同じ濃色**（ピンクにすると猫に見えない）。ほっぺは 2×1 で輪郭に接触させない。⑤しっぽは**短く細い鉤形**——右辺の下（row 17）から真横へ出て col 20–21 で上へ折れ、row 13 で止まる。中身 1 ドット幅・全長は胴体の 1/3 程度。振り（`TAIL.up`/`down`）は**先端に 1 ドット足す／取る**だけで根元は動かさない。⑥**教科小物と番号バッジは載せない**——24px では小物に胴体左下の数ドット、バッジの数字に 3×3 しか取れず、どちらも何が描いてあるか判別できないため。表情は設定資料の 6 種（標準・にっこり・まじめ・うれしい・ひらめき・びっくり）を SVG 資産にしてあり、残り（まばたき・あくび・よそ見・耳ぴく・わくわく）は待機アニメ専用。**口を差し替えるパッチで row 14 をまとめて上書きしない**——鼻（col 9）ごと消えて猫に見えなくなる（`tests/test_fuseneko.py` が全表情で検査する）。**造形定義は外部ファイルなので、読み込みに失敗しても本文が死なないようにする**——`index.html` は `FN_OK` で通し、`setupMascot()` がマスコットだけ諦める。吹き出しの枠色（`mixWithBlack(…, 0.38)`）は `--cat-outline` と同じ比率にすること。造形の確認は `dev/fuseneko.html`（サイズ・表情・色・背景を並べる）。待機アニメは 16 コマのコマ送り（`#mascotStrip` の `translateX`）と、割り切れない周期の呼吸（`#mascotBob` の `translateY`。同一要素で `transform` を二重に持てないので入れ子にしている）を重ねる。**コマの中身は固定ではなく、`animationiteration` のたびに `shuffleCatFrames()` が選び直す**——`CAT_TAIL_TRACKS` からしっぽのリズムを 1 本引き、空きコマへ動きを置く。ふだんの動きは `CAT_ACCENTS`（まばたき／にっこり／びっくり／耳ぴく／よそ見〈左右〉／わくわく／あくび／うれしい／ひらめき）から 2〜3 個、**まれな来客**は `CAT_RARES`（ちょう／とり／ハート／おんぷ）から `CAT_RARE_CHANCE`（7%）で 1 周に最大 1 回。来客はコマ数が多いので先に場所を取る。重みは `CAT_ACCENT_POOL`/`CAT_RARE_POOL` で mood ごとに変える（終了後は「びっくり」を出さず、あくびとおんぷ中心）。動きは 1 要素＝1 コマぶんの**重ねるパッチ配列**なので、来客のようにコマごとに位置が変わるものも書ける（位置が動くパッチは `catRow()` で組み立てる）。**先頭コマには何も置かない**（周の切れ目で表情が飛び込まないように）。置き場所は空きが見つかるまで数回やり直す（1 回で諦めると 2 割強が衝突で消える）。しっぽのリズムは前周と別の 1 本を引く。つつかれたときは `reactCatFrames()` が先頭コマへ反応の絵を入れて**コマ送りを先頭から回し直す**（「いま何コマ目か」を調べずに済む。1 周後の `shuffleCatFrames()` が自然に元へ戻す）。**mood が変わると CSS が周期を差し替えるため、`restartCatAnim()` でコマ送りと呼吸を先頭から仕切り直す**——そのままだと累計経過の再解釈でコマが飛ぶ。16 コマの組み立ては 7ms 前後かかるので `requestAnimationFrame` で初回描画の後ろへ回し、組み上がってから `pop-in` で出す。差し替えは**コマ送りが先頭へ戻った瞬間**なので見た目が飛ばず、変わったコマだけ書き換える（SVG は `catSvgCache` で使い回す）。体の上下は**ふだん静止し、1 周に一度だけ 1 ドット短く沈む**（常時ゆれると「ぶれ」に見えて疲れるため）。沈んでいる時間は 1 周の 8% ほどなので、間隔が短めでも「ぶれ」には見えない。**間隔はほぼ一定**（`jitterCatBob()` が 1 回沈むごとに基準の ±12% で振り直す。既定 5 秒なら 4.4〜5.6 秒）にして規則正しい呼吸に見せ、たまに 2 回続けて沈む型（`catBob2`、12%）を混ぜる。**走行中に `animation-duration` を書き換えると「累計経過 ÷ 新しい周期」で回数が計算し直され、iteration がまとめて消化されて間隔 0 秒で何度も沈む**ので、`animation-name` を一度 `none` にして強制リフローしてから入れ直す。`prefers-reduced-motion` では作り替えない。**アニメーションイベントは親へ上がるので、リスナは必ず `e.animationName` で見分ける**（`#mascotStrip` の catIdle が `#mascotBob` にも届く）。吹き出しの文言と場面は `mascotState(now = new Date())` がイベントデータと現在時刻から算出する**純関数**で、`{ mood, lines }` を返す（`mascotLines(now)` は `lines` だけ返す薄いラッパー。`now` を渡せばコンソールから全分岐を確認できる）。`mood`（`before`/`today`/`live`/`after`/`ended`）は `#mascot` の `data-mood` に載せ、**CSS 側で待機アニメの速さだけを切り替える**（JS のタイマーは増やさない）。吹き出しもドット風で、**フォントは DotGothic16**（Google Fonts。吹き出し専用。ウェイトは 400 のみなので `font-bold` を当てない）、枠と三角のしっぽは `BUBBLE_FRAME`/`BUBBLE_ARROW` から `buildDotDataUri()` が data URI の SVG を作り、`--cat-frame`/`--cat-arrow` 経由で `border-image`（9スライス）と `background` に渡す。色はネコの輪郭と同じ計算（`mixWithBlack(resolvedBrand(), 0.6)`）。**data URI の SVG には `width`/`height` を必ず入れる**——固有サイズが無いと `border-image-slice` の数値が画像のドット数ではなく CSS ピクセル扱いになり 9 分割がずれる。**吹き出しの折り返しは `setMascotText()`（`bubbleHtml()`）が「…」で囲んだ演目名を `.nb`（`word-break: keep-all`）で包んで守る**——包まれた題は 1 行に収まらないとき丸ごと次の行へ送られ、**そのぶん広い幅を貰えて語が無傷のまま収まる**。上限 `BUBBLE_NOBREAK_MAX` は **12 文字**で、これは `events/*.json` の `category` が最長 11 文字（「パネルディスカッション」等）だから——**実データ全部が対象に入る値にする**。これより長い題は包まない（次の行へ送っても収まらず、「いまは」だけが 1 行目に残ったうえで結局は途中で折り返すので、行が増えるだけ損）。`overflow-wrap: break-word` を併記して、収まらないときは枠の外へ出さず改行させる（`white-space: nowrap` では 320px の小画面で枠外へはみ出す）。**長さは escape 前の生テキストで数える**——`&` が `&amp;` に膨らむと 5 文字と数えてしまい、本来包める題（「Q&A講座」）が漏れる。**`text-wrap: balance` を当ててはいけない**——行の長さをそろえようとして「ワークショッ／プ①」のように**語の途中で割る回数がかえって増える**（実データ 50 文言 × 5 画面幅＝250 件で、素の `pretty` 123 件に対し `balance` はさらに悪化。`.nb` 併用の `pretty` は 6 件）。本文の `p { text-wrap: pretty }` を継承させたままにすること。なお `h1,h2,h3` の `balance` は**そのままでよい**——イベント名・演目名で実測したが `pretty` と優劣が付かず（`balance`「全国算／数」対 `pretty`「研究／大会」）、見出し全体の見た目を変えるだけの根拠が無い。表示は**イベントページのみ**で、`eventInfo.mascot: false` を置いたイベントでは出さない。文言は必ずデータから作り、`index.html` に固有文言を書かない（**キャラクター名 `CAT_NAME`＝「ふせんネコ」は例外**——イベント固有ではないので汎用シェルに置く）。**名前は隠し要素として `TAP_GAP_MS`（1.2 秒）以内に `TAPS_TO_INTRODUCE`（3 回）つつくと自分から名乗る**。名乗るときは吹き出しを強制的に開くので、折りたたみの 3 点セット（class・`aria-expanded`・localStorage）は `setupMascot()` の `setFolded()` を必ず通す。自己紹介は `mascotPinUntil`（`MASCOT_PIN_MS`＝8 秒）の間だけ 7 秒タイマーを休ませて残し、`mascotMsgs`/`mascotMsgIdx` は触らないので解除後は次の一言へ自然に戻る。`prefers-reduced-motion` では絵は変えずに文言だけ出す。名前は `#mascotBtn` の `aria-label`／`title` にも入れてある（画面に出る名前はこの 2 箇所だけ）。**ドラッグ（つまんで引っぱる）にも対応する**——`#mascotBtn`（コマ送り・呼吸）とは別に、外側の `#mascotHold` を 3 枚目の transform 層として持ち（同一要素で `transform` を二重に持てないのは他の入れ子と同じ理由）、`--mascot-dx`/`--mascot-dy`（CSS変数、`translate3d`）でずらす。位置は保存しない＝離すと必ず定位置へバネで戻る。ずらし量は**48px 表示＝1ドット2px に揃えて 2px 単位へ丸める**（他のドット表現と粒度を合わせる）。掴んでいる間は専用のコマ組み `CAT_HOLD`（しっぽを振るびっくり顔）を `holdCatFrames()` が 16 コマ全部に敷き、`shuffleCatFrames()` は `catHeld` フラグを見て待機の差し替えを止める。離すと `releaseCatFrames()` が `catHeld` を下ろし、待機アニメへ戻す。勢いよく払う（速度が `FLICK_V` を超える）と、新しい表情は作らず既存の `lookR`/`lookL`/`blink` の組み合わせだけで「目を回す」`dizzy` を `CAT_ACCENTS` に足して見せる（`CAT_ACCENT_POOL` には入れないので待機アニメには出ない）。**タブの左右スワイプ（`swipeEnabled()`）は `#mascotBtn` の上から始まった指を除外する**——除外しないとタブとネコが同じ指を取り合う。ただし `#mascotBtn` の `touch-action` は `none` ではなく **`pan-y`**（`#tabViews` と同じ値）にしてあるので、ネコの上から**縦**に払った指はブラウザの通常スクロールへ委ねられ、**横**に引いた指だけドラッグとして拾う（縦スクロールまで潰さない）。**`touch-action` だけでは足りず、JS 側にも軸判定（`DRAG_AXIS_RATIO`＝1.4、タブスワイプの `AXIS_RATIO` と同値）が要る**——ブラウザが「これは縦スクロールだ」と判断するのは数 px 動いた後で、`DRAG_MIN_PX`（6px）の閾値と競合する。軸判定が無いと、縦に払っただけで一瞬ネコが持ち上がり吹き出しが畳まれてから `pointercancel` で戻る、というちらつきが出る。縦優勢と判定した指は `pressedAt` を降ろして以降の `pointermove` を先頭で抜けさせる（押しっぱなし扱いで「なでる」が誤発火するのも防ぐ）。**隠し要素は他に 3 つ**: ①**長押し（`PET_HOLD_MS`＝0.6 秒、動かさずに押し続ける）で「なでる」**——`reactCatFrames("heart")` でハートの来客を確定で流す（`reactCatFrames()` は `CAT_ACCENTS[name] || CAT_RARES[name]` を引くので来客も同じ枠で流せる）。②**掴んで放すと吹き出しが「離す前の状態」へ静かに戻り**（畳む／保存はしない）、**勢いよく払うと目を回して一言残す**（`mascotPinUntil` で固定）。**なでる・ドラッグとも、離すと `click` が続けて飛んでくるので 1 回だけ捨てる**（`swallowClick`。捨てないと吹き出しが畳まれる／開閉が誤爆する。click が来ない離し方に備えて次の `pointerdown` でも下ろす）。ポインタ系のリスナはドラッグ判定も含めて `{ passive: true }` を保つ（`preventDefault()` は使わず `touch-action` と `setPointerCapture()` に任せる）。③**深夜・早朝の一言**（`withNightLine()`）——`mascotState()` の全分岐が通る `state()` を包んでいるので**足すのはここ 1 箇所だけ**。進行中（`live`）と文言が空のときは足さない。純関数のままなので `mascotLines(new Date("2026-08-01T02:00+09:00"))` で確認できる。**「素早く N 回」の数えは `makeTapCounter(times)` に集約**（`TAP_GAP_MS` 以内なら加算・達したら 0 に戻す）。`reactCatFrames()` は**知らない名前で呼ばれたら黙って返す**——`frames` が undefined のまま進むと例外で呼び出し元（吹き出しの開閉）ごと死ぬため。（**「ふせんネコ図鑑」は一度試作したが、静止 1 コマでは表情差分が分かりにくく削除した**。作るなら待機アニメと同じ複数コマのアニメで見せること。）
...
- **画像と転送量（会場の細い回線が前提）**: 自前のコードは軽い（`index.html` は gzip 53KB、
  ...
  **JSON は書き換えない**。未知のホスト・想定外の形は素通しし、派生が 404 のときは
  ...
  数値は持てない——`aspect-[2/1]` で比率だけ固定する。会場マップ（`#venueMap`）も同様に実寸が
  一定しないので `aspect-square object-contain`（現行 2 枚は 1349×1459 / 1529×1625 とほぼ 1:1。
  比率が違っても歪まず、余白は地の色になる）。**`loading="lazy"` の画像を「読み込めるまで隠す」
  作りにしてはいけない**——`display:none` の間はビューポートに入りようがないので取得自体が
  始まらず、`load` が永久に来ない＝画像が二度と出ない。隠すのは `error` のときだけにして、
  ちらつきは箱の先取りで受ける。④`<head>` のフォント CSS は
...
| `dev/nav-glass.html` | ボトムナビ（Liquid Glass）の確認ページ。Tailwind CDN 無しで動くよう該当ユーティリティを手写ししてある。`?brand=%23f59e0b` でブランド色を差し替え | △ ナビの見た目を変えたら CSS を写し直す |
| `dev/now-card.html` | 進行中カードの演出（回る縁・ドットの波）の確認ページ。Tailwind CDN 無しで動くよう該当ユーティリティを手写ししてある。ブランド色は画面上のボタンか `?brand=%23005860` で切り替え | △ 進行中カードの CSS を変えたら写し直す |
...
- 例外的に手動で状態を固定したい場合のみ、個別 JSON ルート直下に `"_status": "ended"` または `"active"` を置く（あれば日付判定より優先。中止・延期・恒久保留などの特殊ケース用）。`scripts/find_event.py` の終了判定と、UI ではマスコットの場面（`mascotState()` の `ended`）だけが見る。
