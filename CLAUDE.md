...
- **マスコット（右下のドット絵ネコ）**: ドット絵は `CAT_BASE`（24×24 の文字列配列。ふせん本体は col 2–17 / row 5–20 の**正方形**）＋差分パッチ `CAT_TAIL_UP`/`CAT_TAIL_DOWN`（しっぽの振り）/`CAT_BLINK` で持ち、`buildCatSvg()` が実行時に SVG を生成する（バイナリ資産なし）。パッチは**セル単位で重ねられる**形式で、`_` は「下の絵を残す」・`.` は「透過にする」を意味する（`applyCatPatches()` が合成。しっぽの振りとまばたきを同じコマに同時適用できる）。色は `<style>` の `.cat-*` クラス側にあり、輪郭・目・口は同系の濃色 1 色（`color-mix(var(--brand) 70%, black)`）、体は `color-mix(var(--brand) 52%, white)` の淡いパステルで、いずれも `--brand` から作るのでイベント毎に自動追従する。**黒に近い固定色は使わない**（重く見えるため）。吹き出しの文言は `mascotLines(now = new Date())` がイベントデータと現在時刻から算出する**純関数**（`now` を渡せばコンソールから全分岐を確認できる）。表示は**イベントページのみ**で、`eventInfo.mascot: false` を置いたイベントでは出さない。文言は必ずデータから作り、`index.html` に固有文言を書かない。
