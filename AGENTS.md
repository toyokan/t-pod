...
- **マスコット（右下のドット絵ネコ）**: ドット絵は `CAT_BASE`（24×24 の文字列配列）＋差分 `CAT_TAIL_UP`/`CAT_BLINK` で持ち、`buildCatSvg()` が実行時に SVG を生成する（バイナリ資産なし）。色は `<style>` の `.cat-*` クラス側にあり、輪郭と体色は `--brand` から作るのでイベント毎に自動追従する。吹き出しの文言は `mascotLines(now = new Date())` がイベントデータと現在時刻から算出する**純関数**（`now` を渡せばコンソールから全分岐を確認できる）。表示は**イベントページのみ**で、`eventInfo.mascot: false` を置いたイベントでは出さない。文言は必ずデータから作り、`index.html` に固有文言を書かない。
