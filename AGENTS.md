...
- **マスコット（右下のドット絵ネコ）**: ドット絵は `CAT_BASE`（24×24 の文字列配列。ふせん本体は col 2–17 / row 5–20 の**正方形**）＋差分パッチ `CAT_TAIL_UP`/`CAT_TAIL_DOWN`（しっぽの振り）/`CAT_BLINK`/`CAT_SMILE`/`CAT_SURPRISE`/`CAT_EARS_DOWN` で持ち、`buildCatSvg()` が実行時に SVG を生成する（バイナリ資産なし）。パッチは**セル単位で重ねられる**形式で、`_` は「下の絵を残す」・`.` は「透過にする」を意味する（`applyCatPatches()` が合成。しっぽの振りとまばたきを同じコマに同時適用できる）。色は `<style>` の `.cat-*` クラス側にあり、**体（`--brand` 52%＋白）→ 輪郭（60%＋黒）→ 目・口（50%＋黒）の 3 段**で、いずれも `--brand` から作るのでイベント毎に自動追従する。**黒に近い固定色は使わない**（重く見えるため）。待機アニメは 16 コマのコマ送り（`#mascotStrip` の `translateX`）と、割り切れない周期の呼吸（`#mascotBob` の `translateY`。同一要素で `transform` を二重に持てないので入れ子にしている）を重ねる。吹き出しの文言と場面は `mascotState(now = new Date())` がイベントデータと現在時刻から算出する**純関数**で、`{ mood, lines }` を返す（`mascotLines(now)` は `lines` だけ返す薄いラッパー。`now` を渡せばコンソールから全分岐を確認できる）。`mood`（`before`/`today`/`live`/`after`/`ended`）は `#mascot` の `data-mood` に載せ、**CSS 側で待機アニメの速さだけを切り替える**（JS のタイマーは増やさない）。表示は**イベントページのみ**で、`eventInfo.mascot: false` を置いたイベントでは出さない。文言は必ずデータから作り、`index.html` に固有文言を書かない。
