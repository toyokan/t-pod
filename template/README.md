...
| --- | --- | --- |
| **基本情報** | `eventInfo` ＋ `events.json` | 縦並び。研究会名・テーマ・会場・ブランド色(16進)・イベントID(半角英数/ハイフン/アンダースコア)・並び順日付(YYYY-MM-DD)・LINE追加ポップアップ(true/false)。**主催／共催／協力／後援**は該当があるものだけ記入（空欄の行は表示されない）。下部に「開催日ごと(dates)」「会場一覧(rooms)」「任意設定」の小表。任意設定では補助ブランド色、会場マップ画像、書籍特設販売リンクを指定できる。 |
...
- **基本情報** → `eventInfo` の `title`/`subtitle`/`tagline`/`logoMain`/`logoSub`/`theme`/`venueName`/`dateRange`/`brandColor`/`brandColor2`/`linePromo`（`true` のときだけ真偽値で出力。`false`・空欄は省略可）、`venue.mapNote`/`venue.mapImage`、`bookSale`（`url`/`label`/`note`）。空欄の任意項目は出力しない。`dates[]`（`id` は day1/day2… を自動採番、`label`/`date`/`weekday`/`time`）、`rooms[]`（`id`=`name`=会場名、`color`）。`events.json` には `id`/`title`/`theme`/`dateRange`/`venueName`/`sortDate`(=並び順日付)/`brandColor` を1エントリ追記。
