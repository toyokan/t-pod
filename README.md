...
- **イベント毎にテーマ色を変更可能**。`eventInfo.brandColor` にメイン1色を指定するだけで全体の濃淡を自動生成。**背景色の輝度に応じて文字色を白／濃紺に自動切替**（`--brand-fg`）。ただしこの判定は輝度の閾値によるもので万能ではなく、amber・黄など明るい色ではブランド色ベタ塗りに白文字が乗る箇所が WCAG AA に届かない。ボトムナビ・日付タブは**白ピル＋ブランド濃色文字**（`brand-800` 相当）に変更済みで、どのテーマ色でも 5.5:1 以上を確保。**新しい配色を足すときはブランド色ベタ塗り＋白文字を避け、この方式に倣う**（残りの未対応箇所は `progress.md` の残課題を参照）
- **スクロールで縮小するヘッダー**。下スクロールでタイトルをコンパクト化し、画面を有効活用（`prefers-reduced-motion` 環境では即時切替）
- **左右スワイプでタブ移動**。スマホで TIMETABLE / FILES / BOOKS を横スワイプで切替。指追従はせず「離した時点で確定」する方式のため、タッチ監視は `passive` のままで**縦スクロールを妨げない**。両端では止まり（ループしない）、横スクロールする会場列・リンク・ボタンの上、モーダル表示中、一覧ページでは無効
- **軽量なアニメーション**。タブ切替はスワイプ方向へスライド＋フェード、ボトムナビと日付タブの選択表示はインジケータが滑らかに移動。すべて `transform`／`opacity` のみで GPU 合成し、`prefers-reduced-motion` 環境では停止
- **すりガラス調のボトムナビ／日付タブ**（半透明＋`backdrop-blur`）。`backdrop-filter` 非対応の環境では不透明の塗りへ自動フォールバックするため、可読性は落ちない
