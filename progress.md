...
- [x] **ボトムナビの選択タブのコントラスト**（当初は白ピル＋ブランド濃色文字、現在は補正済みブランド背景＋白文字。全テーマ色で 4.5:1 以上）。
- [x] **`--brand-fg`（ブランド色ベタ塗り＋白文字）の残り**: 2026-07-16 に案Bを実装。`--brand-solid` を白文字 4.5:1 以上になる最も明るい黒混合色として二分探索で生成し、ヘッダー、`grad-pill`、進行中表示、ジャンプボタン、インストール導線、書籍販売CTA、ボトムナビへ適用。識別用の `--brand` は変更しない。非アクティブ `cat-chip` は `brand-700` から `brand-800` へ変更。
  ...
  - **採用方針**: `--brand` は識別色（ドット・チップ・枠線・濃色文字）としてそのまま使い、白文字が乗る塗りだけ補正済みの `--brand-solid` を参照する。
...
- 切替閾値を 60px から 48px に短縮。未達・両端方向では 160ms で元位置へ戻す。
- ボトムナビの選択ピルを白背景＋ブランド濃色文字から、`--brand-solid` のブランド背景＋白文字へ変更。全ブランド色で本文相当の 4.5:1 を保証する。
