...
- [x] **ボトムナビの選択タブのコントラスト**（グレーのトラック＋白ピル＋ブランド濃色文字で解消。全テーマ色で 5.5:1 以上）。
- [ ] **`--brand-fg`（ブランド色ベタ塗り＋白文字）の残り**: ボトムナビ・日付タブは対応済みだが、以下は未対応で明るい `brandColor` では AA 未達のまま。
  ...
  - **方針（2026-07-16 合意・未着手）**: 「**ブランド色の選択に制限を付ける／指定色をサイト実装時に適切な範囲へ入れ替える**」方向で対応する。`--brand` は識別色（ドット・チップ・枠線・濃色文字）としてそのまま使い、**白文字が乗る塗り用に補正済みの色（仮称 `--brand-solid`）を JS で自動生成**して、ヘッダー・`grad-pill` 等はそちらを参照する案（下記「案B」）が本命。
