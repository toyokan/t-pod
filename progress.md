...
- 現在のデザイン: ロゴ調のコンパクトなヘッダー（**スクロールで縮小**）、フッターは **TIMETABLE / FILES / BOOKS** の3タブ。テーマ色は `eventInfo.brandColor` で**イベント毎に変更可能**（濃淡は自動生成、**背景色の輝度に応じて文字色 `--brand-fg` を白/濃紺に自動切替**）。**一覧ページの基調色は東洋館イメージカラーのイエロー `#ffd900`**、一覧の各カードは `events[].brandColor` でイベント固有色に切替。ヘッダー2行目は `title`（正式名称）優先、テーマは `――` 等の区切りでサブテーマを自動改行。**クレジット表記（`App by TOYOKAN PUBLISHING CO., LTD.`）を全タブ・一覧ページの本文最下部に控えめに表示**。概要・お知らせの1つ目のカードに**主催／共催／協力／後援**（`organizer` ほか・指定があるものだけ）を表示。
